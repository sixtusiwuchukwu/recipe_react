      ...
      <form onSubmit={recipeData ? handleUpdate : handleFormSubmit}>
        ...
        <IngredientsContainer>
          {ingredients.map((ingredient, index) => (
            <div key={index} style={{ display: 'flex',alignContent:'center',alignItems:'center',marginTop:'10px'}}>
              <InputField
                type="text"
                value={ingredient}
                ...
                required
              />
              <RemoveButton type="button" onClick={() => removeIngredientField(index)}>
                Remove
              </RemoveButton>
            </div>
          ))}
        ...
        />
          ...
          <Label htmlFor="image" style={{marginTop:'30px'}}>Upload Image</Label>
          <Input type="file" id="image" accept="image/*" onChange={handleImageChange} />
          {imagePreview && <RecipeImage src={imagePreview} alt="Recipe Preview" />}
        </ImageUploadContainer>
        <SubmitButton type="submit">{recipeData ? 'Update Recipe' : 'Submit Recipe'}</SubmitButton>
      </form>
    </FormContainer>
  );
};
